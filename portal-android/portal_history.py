"""
История Portal на Android (SQLite рядом с portal_android_config.json).
Логика таблицы синхронна с ../portal_history.py при изменениях схемы.
"""
...
def history_db_path() -> Path:
    try:
        ...
        p = Path(app_storage_path()) / "portal_history.sqlite3"
        p.parent.mkdir(parents=True, exist_ok=True)
        return p
    except Exception:
        p = Path.home() / ".portal_android" / "portal_history.sqlite3"
        p.parent.mkdir(parents=True, exist_ok=True)
        return p
