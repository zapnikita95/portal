...
WIDGET_PRESET_EVENT_LABELS_EN: Dict[str, str] = {
    "receive": "Clipboard / rich receive",
    "receive_file": "File receive",
    "send": "Send to remote PC",
}
...
STRINGS: Dict[str, Dict[str, str]] = {
    "ru": {
        ...
        "recv.per_ip_title": "Отдельная папка приёма по IP отправителя (необязательно):",
        "recv.per_ip_hint": "Строка: IP<TAB>путь или «IP пробел путь». IP как в логе при приёме (например 100.x.x.x).",
        "recv.save_ip_list": "Сохранить\nсписок IP",
        ...
        "widget.rules_hint": "Правила: IP получателя или * → когда сработало → какой пресет. Сначала совпадёт конкретный IP, потом *. Если для «Приём файла» нет строки — используется правило для «Приём в буфер».",
    ...
    "en": {
        ...
        "recv.per_ip_title": "Per-sender receive folder (optional):",
        "recv.per_ip_hint": "Line: IP<TAB>path or “IP path”. IP as in receive log (e.g. 100.x.x.x).",
        "recv.save_ip_list": "Save\nIP list",
        ...
        "widget.rules_hint": "Rules: recipient IP or * → event → preset. Specific IP first, then *. If no row for “File receive”, “Clipboard receive” rule is used.",
