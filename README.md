...
- 💾 **Несколько IP**: список в главном окне → «Сохранить список IP», затем **галочки — кому слать сразу** → «Сохранить выбор». Файлы и буфер уходят на **все отмеченные** машины. Конфиг: `%APPDATA%\Portal\config.json` (Win) или `~/Library/Application Support/Portal/config.json` (Mac), поля `peer_ips`, `peer_send_targets`
