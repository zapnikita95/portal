...
- ⌨️ **Горячие клавиши**:
  ...
  - **Общий буфер**: забрать буфер с другого ПК (текст / файлы / картинка) — `Ctrl+Alt+V` (Win) / `Cmd+Shift+V` или `Cmd+Option+V` (Mac)  
    *(на втором компьютере должен быть запущен приёмник «Запустить портал», в настройках указан IP пары)*
- 🖱️ **Виджет**: перетаскивание — **Alt + левую кнопку**; файл — **Ctrl + клик**; **двойной клик** — картинка из буфера или выбор файла; IP — **ПКМ** → «IP удалённого ПК»
...
- 📁 **Куда сохранять входящие файлы** + **режим для файлов из буфера** (только папка / только буфер / оба) — в главном окне; из виджета — **ПКМ** → «Приём файлов из буфера…» (по умолчанию папка — **Рабочий стол**, с учётом OneDrive на Windows)
- 🖼 **Двойной клик по виджету-порталу** — если в буфере **картинка**, она уходит на пару как PNG; иначе открывается выбор файла (как Ctrl+клик). IP — через ПКМ → «IP удалённого ПК»
