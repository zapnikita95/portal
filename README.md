...
## 🔧 Диагностика связи (ping → pong)

Если индикатор «Пара» показывает таймаут, а на втором ПК портал запущен:

1. **На Mac** (принимающая сторона): при нажатии «Проверить» в журнале должна появиться строка `📡 Ping от X.X.X.X → отправляю pong`. Если её нет — подключение до Mac не доходит.
2. **Ручная проверка** из терминала (Windows или Mac):
   ```bash
   python test_probe.py 100.65.63.84
   ```
   Скрипт покажет шаг, на котором произошла ошибка (connect, send, recv).
3. **Tailscale**: `tailscale ping 100.65.63.84` — проверить маршрутизацию.
4. **Версии**: на обоих ПК должен быть код с `ping/pong` (свежий `git pull`).
