...
- **Куда сохраняются файлы (Android):** основная копия — во внешнюю папку приложения (`Android/data/…/files/PortalReceive`, если поле «Папка приёма» пустое). Дополнительно автоматически создаётся **копия в системных «Загрузки» → папка `Portal`** (видно в приложении «Загрузки»), через локальный плагин `portal_android_downloads` (MediaStore). Своя папка из «Выбрать папку…» проверяется при сохранении настроек — с фона Android запись туда часто **невозможна**; если проверка не прошла, оставь поле пустым.
...
**LAN «Найти в LAN»:** на экране **Пиры** — **Wi‑Fi**, **mesh** или **Все**. Параллельно: **mDNS** (`_portal._tcp`, как на десктопе) и **TCP-скан** по сегментам; для **mesh** в перебор входят и 100.64–127.x, и домашняя LAN при наличии Wi‑Fi IP или подсказки. Пока на телефоне включён **приём**, он тоже **анонсируется** в mDNS (Bonsoir), имя — в **Настроить** (`portal_mdns_display_name` в духе ПК). В репозитории уже есть `android/`; при пересоздании проекта прогони `python3 tool/patch_android_manifest.py` и `patch_android_gradle.py`.
