...
**LAN «Найти в LAN»:** на экране **Пиры** — **Wi‑Fi**, **mesh** или **Все**. Скан идёт **с телефона**; для **mesh** в перебор входят и адреса mesh-VPN (100.64–127.x), и домашняя LAN (если на телефоне есть Wi‑Fi IP или подсказка/пир в 192.168…). В репозитории уже есть `android/`; при пересоздании проекта прогони `python3 tool/patch_android_manifest.py` и `patch_android_gradle.py`.
